...
function Application({
  ...
  const validationSchema = Yup.object().shape({
    phone: Yup.string().required('Введите номер телефона').matches(phoneRegex, 'Некорректный номер телефона'),
  ...
  const { values, handleBlur, handleChange, handleSubmit, errors, touched, resetForm } = useFormik({
    ...
    onSubmit: (values) => {
      const req: sendApplicationData = {
        ...
        parent_model: model
          ? model
          : pathname[1] && pathname[1] === 'autocredit'
            ? 'autocredit'
            : models[pathname[0]].parentModel,
        model: model
          ? model
          : pathname[1] && pathname[1] === 'autocredit'
            ? 'autocredit'
            : models[pathname[0]].childModel,
        product_name,
        bank_name,
      ...
      gtagEvent('submit', model ?? models[pathname[0]].parentModel);
      dispatch(
        sendApplication({
          cb: () => {
            onSuccessSendApplication();
            gtagEvent('success', model ?? models[pathname[0]].parentModel);
          },
          data: req,
        }),
      );
      ...
      >
        <Box sx={style}>
          ...
          >
            ...
            >
              Оставьте заявку и получите бесплатную консультацию от нашего финансового эксперта
            </Typography>
            <Grid component='form' container mt='20px' onSubmit={handleSubmit} sx={formStyles}>
